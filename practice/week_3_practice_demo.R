...
ggplot(county_data)+
  aes(x=estimate, y = moe_pct, color = "blue")+ # this is the example of a wrong way of customizing the visualization
  geom_point()
...
county_data %>%
  ...
  geom_col() +
  geom_errorbar(aes(ymin = estimate - moe, ymax = estimate + moe)) # errorbars have an upper and lower limit (how big you should make the error bars). You should never turn in a graph that looks like this.
...
county_data %>%
  ...
  geom_errorbar(aes(ymin = estimate - moe, ymax = estimate + moe), width = 0.3, color = "steelblue") +
  coord_flip() +
...
county_data %>%
  ...
  ggplot(aes(x = reorder(NAME, estimate), y = estimate)) +
  geom_col(fill = "lightblue") +
  geom_errorbar(aes(ymin = estimate - moe, ymax = estimate + moe), width = 0.6, color = "steelblue") +
  coord_flip() +
  theme_minimal() +
...
region_total_moe <- moe_sum(region$moe, region$estimate) # specifying the moe data column in that df and the estimate of it. moe_sum does the complicated maths for you (of squaring and sqrt)
...
get_poverty <- function(yr) { # this is how functions work in r. SO if you put get_poverty(2017)
  get_acs(
    geography = "county",
    variables = "B07012_002",
    state     = "NY",
    ...
    mutate(period = paste0(yr - 4, "-", yr)) # full four year time period.
}

# this is in long format (because you are scrolling for a long time). Switch to wide
